    ...
    def get_new_point_in_ellipsoid(self, goal_bias, c_best):
        ...
        while not valid:
            ...
            # Generate a random point in an ellipsoid
                ...
                center = [[(self.goal.row+self.start.row)/2,],[ (self.goal.col+self.start.col)/2]]

                # Compute rotation matrix from elipse to world frame - C
                theta = atan2((self.goal.row - self.start.row), (self.start.col - self.goal.col))
                # print(theta)
                R = np.matrix([[cos(theta), -sin(theta)], [sin(theta), cos(theta)]])


                # Compute diagonal matrix - L
                l = np.diag([ ((c_best**2 - c_min**2)**0.5)/2, c_best/2])
                l = np.matrix(l)

                # Cast a sample from a unit ball - x_ball
                x_ball = self.sample_unit_ball()
                x_ball = np.matrix(x_ball)
                # print(x_ball)

                # Map ball sample to the ellipsoid - x_rand
                point = np.dot(np.dot(R, l), x_ball) + np.matrix(center)
                point = [int(point[0]), int(point[1])]
                # print(point)

            if point[0]<0 or point[0] >= self.size_row or point[1]<0 or point[1] >= self.size_col:
                valid = False
            elif self.map_array[point[0]][point[1]] == 0:
                valid = False
            else:
                valid = True
